...
**暂缓冻结：** 阿里云资产值、固定公网IP实际值、异机备份目标和正式部署命令

### V1.4部署方向变更

项目负责人已确认第一版使用阿里云固定公网IPv4直接访问，不配置域名、DNS和TLS证书。
V1.4替换V1.3中的“域名+HTTPS”方向，冻结为“阿里云固定公网IPv4+HTTP的Docker Compose单机部署”。
本次变更不修改业务范围、数据模型、Excel规则、权限模型或SQLite单实例约束。
...
> 阿里云固定公网IPv4＋HTTP＋Docker Compose＋Nginx单机部署。
...
Nginx为第一版必选组件，Django端口不得直接暴露到公网。第一版只使用80端口，
不配置域名、DNS、TLS证书或443端口。详细部署决策见`docs/deployment_decisions.md`。

HTTP不提供传输加密。TCP 80必须优先限制为学校、负责人或实际使用人员的固定公网IP/CIDR；
如果80端口需要向全网开放，只允许脱敏演示，不得导入真实学生数据。
...
* 生产模式缺少密钥、固定公网IPv4或匹配的HTTP可信来源时拒绝启动；
* 使用Nginx作为唯一公网入口；
* 使用生产级WSGI服务，不以`runserver`上线；
* 将SQLite、媒体、静态文件和备份挂载到持久化目录；
* 提供健康检查、部署检查和容器构建验证。
...
系统正式运行期间至少每日备份一次；每次正式Excel导入前后额外备份。
...
异机备份为公网云服务器部署的必选要求，具体目标和访问凭据在授权运维台账登记，不写入仓库。
...
* 日期规则无误；
* 页面采用简约高校年轻风格；
* 正式部署采用阿里云固定公网IPv4、HTTP、Docker Compose和Nginx方向；
* 第一版保持单机、单Django实例和SQLite；
* Nginx为唯一公网入口；
* 正式数据目录必须持久化并执行每日与异机备份。
...
15. SQLite数据库；
16. Django单体架构；
17. 阿里云固定公网IPv4、HTTP、Docker Compose和Nginx部署方向；
18. 可迁移部署与操作指南要求；
19. 简约高校年轻视觉风格；
20. 七人功能闭环分工方式。
