...
**暂缓冻结：** 云厂商资产值、正式域名、证书签发信息、异机备份目标和正式部署命令

### V1.3部署方向变更

项目负责人已确认第一版改为购买的公网云服务器部署。V1.3替换V1.2中的
“校园内网单机部署”方向，冻结为“公网云服务器上的Docker Compose单机部署”。
...
> 公网云服务器＋Docker Compose＋Nginx＋HTTPS单机部署。
...
Nginx为第一版必选组件，Django端口不得直接暴露到公网。HTTP端口只用于证书签发和
跳转HTTPS；正式业务访问必须使用HTTPS。详细部署决策见`docs/deployment_decisions.md`。
...
* 日期规则无误；
* 页面采用简约高校年轻风格；
* 正式部署采用公网云服务器、Docker Compose、Nginx和HTTPS方向；
...
15. SQLite数据库；
16. Django单体架构；
17. 公网云服务器、Docker Compose、Nginx和HTTPS部署方向；
18. 可迁移部署与操作指南要求；
19. 简约高校年轻视觉风格；
20. 七人功能闭环分工方式。
