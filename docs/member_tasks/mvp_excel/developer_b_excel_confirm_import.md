...
快照、SQLite备份和正式写入必须位于同一个导入串行化保护窗口：

1. 获取项目唯一的导入串行化控制后，重新读取批次，不沿用锁外对象。
2. 在任何业务写入前完成原文件、preview、候选、rollback快照和SQLite备份校验。
3. 进入正式业务事务后再次校验批次仍为`previewed`，原文件与preview证据未变化。
4. 在事务内完成全部业务写入、统计、批次`success`状态和`confirm_import`审计，然后一次提交。
5. 无论成功或失败都可靠释放串行化控制。

实现可使用适合单机单实例SQLite的文件锁、进程锁或经评审的等价方案，但不得只依赖SQLite无效的行级锁假设。锁文件必须位于受控运行目录，使用超时和异常安全释放，不得提交到仓库。Module Notes必须说明多进程部署边界；若生产采用多个WSGI进程，所选锁必须能跨进程生效。

### 9.2 事务外失败状态

业务事务失败时先确保事务完全回滚，再在独立事务中把批次标为`failed`并保存安全错误摘要。该步骤不得删除或重写原始文件、preview、rollback快照、SHA、SQLite备份、错误或警告记录；若失败状态本身无法保存，必须记录服务端错误并保持业务表零写入，不能伪装为成功。
...
- 在同一成功事务内记录一次`confirm_import`审计，target指向批次；
- 审计创建失败必须使正式导入事务回滚，不能留下“导入成功但无审计”的状态。
...
- Student成功但材料失败时全部撤销。
- 两个近同时确认只有一个成功。
- 不同批次近同时确认不会形成交叉快照或备份与业务状态不一致。
- 串行化控制超时或异常时业务零写入且锁可恢复。
