...
- 在同一成功事务内记录`rollback_import`；审计失败必须撤销业务恢复和批次状态变化。
...
PR3合入后，A与B共同在同一最终SHA使用约1500条合成数据完成：上传、预览、确认、管理员查询、学生查询、第二次导入、最近批次回滚、查询恢复、证据保留、失败恢复和服务重启后持久性端到端验收。数据覆盖、报告字段和上线门禁以本目录`README.md`为准；联合验收通过后只能进入上线收尾，不得直接声明上线。
