...
本文件仅用于追溯Sprint 2早期任务、Review背景和旧方案来源，不承担以下任何职责：

- 不分配当前开发工作；
- 不定义PR边界、开工条件或验收门禁；
- 不冻结当前公共接口、JSON schema或文件目录；
- 不覆盖`docs/integration_contracts/`、2026-08-12决策或`mvp_excel/`任务卡；
- 不得因本文件保留旧术语而新增`ImportChangeRecord`、数据库快照模型或迁移。

如本文件与现行文件冲突，必须直接采用较新的冻结契约和`mvp_excel/`任务卡，不得折中实现或为旧方案编写兼容层。除修正链接、标注历史状态或补充追溯信息外，不再更新本文件的任务正文。
