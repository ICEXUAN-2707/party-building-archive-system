...
| 核验日期 | 2026-08-22 |
| 集成分支 | `develop` |
| 核验 SHA | `e1ff5f7146213e52fdcf1c64032c46434bbbb8ab` |
| 最新合并 | PR #22，受控SQLite灾难恢复 |
| 本地验证 | PR #22双平台CI通过；PR23数据集契约4项、45条同路径烟测及全仓库342项测试通过；正式1500条报告待固定SHA提交后执行 |
| 业务规范 | `docs/spec.md` |
...
| 最近成功批次回滚与备份 | 已合入 | PR #21业务回滚；PR #22灾难恢复 |
| 约1500条联合验收 | PR23实现候选 | 固定种子生成器、同路径执行器和重启复查已实现，待正式报告 |
...
| `import_rollback_contract.md` | 现行决策已冻结 | PR3阶段一至六已合入 |
...
管理员模块及Excel PR1、PR2、PR3已经合入。当前推进约1500条合成数据同路径联合验收；通过后进入Docker、生产WSGI、持久化和干净环境部署演练。真实数据到达前不声明最终数据验收完成。
